...
status_t MediaSender::queueAccessUnit(
    ...
    if (mMode == MODE_UNDEFINED) {
        ALOGE("MediaSender::queueAccessUnit, INVALID_OPERATION");
        return INVALID_OPERATION;
    }

    if (trackIndex >= mTrackInfos.size()) {
        ALOGE("MediaSender::queueAccessUnit, size error");
        return -ERANGE;
    }

    if (mMode == MODE_TRANSPORT_STREAM) {
        ALOGE("MediaSender::queueAccessUnit, MODE_TRANSPORT_STREAM, accessUnit size %d, trackIndex %d", accessUnit->size(), trackIndex);
        TrackInfo *info = &mTrackInfos.editItemAt(trackIndex);
        ...
        for (;;) {
            ...
            for (size_t i = 0; i < mTrackInfos.size(); ++i) {
                ...
                ALOGE("MediaSender::queueAccessUnit, size: %d", mTrackInfos.size());

                if (info.mAccessUnits.empty()) {
                    ALOGE("MediaSender::queueAccessUnit, info.mAccessUnits.empty()");
                    minTrackIndex = -1;
                    minTimeUs = -1ll;
                    break;
                }
            ...
            if (minTrackIndex < 0) {
                ALOGE("MediaSender::queueAccessUnit, MODE_TRANSPORT_STREAM, minTrackIndex < 0");
                return OK;
            }

            ALOGE("MediaSender::queueAccessUnit, MODE_TRANSPORT_STREAM 2");
            ...
            ALOGE("packetizeAccessUnit, err %d", err);
    ...
    TrackInfo *info = &mTrackInfos.editItemAt(trackIndex);
    ALOGE("info->mSender->queueBuffer");
