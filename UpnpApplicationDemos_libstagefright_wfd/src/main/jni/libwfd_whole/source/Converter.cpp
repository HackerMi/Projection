...
void Converter::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatMediaPullerNotify:
        {
            ...
            ALOGE("Converter::onMessageReceived, kWhatMediaPullerNotify");

            if (!mIsPCMAudio && mEncoder == NULL) {
                ALOGV("got msg '%s' after encoder shutdown.",
                      msg->debugString().c_str());
                ALOGE("Converter::onMessageReceived, kWhatMediaPullerNotify, !mIsPCMAudio && mEncoder == NULL");
            ...
            ALOGE("Converter::onMessageReceived, kWhatMediaPullerNotify 2");

            if (what == MediaPuller::kWhatEOS) {
                ALOGE("Converter::onMessageReceived, kWhatMediaPullerNotify MediaPuller::kWhatEOS");
                ...
                ALOGE("Converter::onMessageReceived, kWhatMediaPullerNotify MediaPuller::kWhatAccessUnit");
                ...
                if (mNumFramesToDrop > 0 || mEncodingSuspended) {
                    ...
                    accessUnit->setMediaBufferBase(NULL);
                    ALOGE("Converter::onMessageReceived, kWhatMediaPullerNotify mNumFramesToDrop > 0 || mEncodingSuspended");
                    break;
                }
...
#endif
                ALOGE("Converter::onMessageReceived, kWhatMediaPullerNotify 3");
...
#endif
            ALOGE("Converter::onMessageReceived, kWhatEncoderActivity");
...
status_t Converter::feedEncoderInputBuffers() {
    ...
    ALOGE("Converter::feedEncoderInputBuffers 1, mInputBufferQueue.empty() %d, mAvailEncoderInputIndices.empty() %d" , mInputBufferQueue.empty(), mAvailEncoderInputIndices.empty());
    ...
    while (!mInputBufferQueue.empty()
        ...
        ALOGE("Converter::feedEncoderInputBuffers queueInputBuffer");
    ...
    ALOGE("Converter::feedEncoderInputBuffers end");
...
status_t Converter::doMoreWork() {
    ...
    ALOGE("Converter::doMoreWork 1");

    if (!(mFlags & FLAG_USE_SURFACE_INPUT)) {
        ALOGE("Converter::doMoreWork 2");
        for (;;) {
            size_t bufferIndex;
    ...
    for (;;) {
        ...
        if (err != OK) {
            ...
            ALOGE("Converter::doMoreWork 4, err %d", err);
            break;
        }
        ...
        if (flags & MediaCodec::BUFFER_FLAG_EOS) {
            ...
            if (flags & MediaCodec::BUFFER_FLAG_CODECCONFIG) {
                ...
                notify->post();
                ALOGE("Converter::doMoreWork 6 send data");
            }
        }

        if (!handle) {
            ALOGE("Converter::doMoreWork 7");
            mEncoder->releaseOutputBuffer(bufferIndex);
        }

        if (flags & MediaCodec::BUFFER_FLAG_EOS) {
            ALOGE("Converter::doMoreWork 8");
            break;
        }
    }
