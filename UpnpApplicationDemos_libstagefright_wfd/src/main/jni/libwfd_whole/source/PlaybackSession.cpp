...
void WifiDisplaySource::PlaybackSession::onMessageReceived(
    ...
    switch (msg->what()) {
        ...
        {
            ALOGE("Received PlaybackSession::onMessageReceived kWhatConverterNotify");
            if (mWeAreDead) {
            ...
            if (what == Converter::kWhatAccessUnit) {
                ALOGE("Received PlaybackSession::onMessageReceived kWhatConverterNotify kWhatAccessUnit");
                ...
                ALOGE("Received PlaybackSession::onMessageReceived kWhatConverterNotify kWhatEOS");
                ...
                ALOGE("Received PlaybackSession::onMessageReceived kWhatConverterNotify kWhatShutdownCompleted");
        ...
        case kWhatMediaSenderNotify:
        {
            int32_t what;
            CHECK(msg->findInt32("what", &what));
            ALOGE("Received PlaybackSession::onMessageReceived kWhatConverterNotify kWhatMediaSenderNotify");
        ...
        case kWhatTrackNotify:
        {
            int32_t what;
            CHECK(msg->findInt32("what", &what));
            ALOGE("Received PlaybackSession::onMessageReceived kWhatConverterNotify kWhatTrackNotify");
        ...
        case kWhatPause:
        {
            ALOGE("Received PlaybackSession::onMessageReceived kWhatConverterNotify kWhatPause");
            if (mExtractor != NULL) {
        ...
        case kWhatResume:
        {
            ALOGE("Received kWhatResume request. 1");
            if (mExtractor != NULL) {
                schedulePullExtractor();
            }
            
            ALOGE("Received kWhatResume request. 2");

            if (!mPaused) {
                break;
            }

            ALOGE("Received kWhatResume request. 3");

            for (size_t i = 0; i < mTracks.size(); ++i) {
                mTracks.editValueAt(i)->resume();
            }

            ALOGE("Received kWhatResume request. 4");
        ...
        case kWhatPullExtractorSample:
        {
            ...
            ALOGE("PlaybackSession::kWhatPullExtractorSample");
...
void WifiDisplaySource::PlaybackSession::schedulePullExtractor() {
    ...
    ALOGE("PlaybackSession::schedulePullExtractor");
...
void WifiDisplaySource::PlaybackSession::onPullExtractor() {
    ...
    ALOGE("PlaybackSession::onPullExtractor");
    status_t err = mExtractor->readSampleData(accessUnit);
    if (err != OK) {
        // EOS.
        ALOGE("PlaybackSession::onPullExtractor EOS");
        return;
    }
    ...
    ALOGE("PlaybackSession::onPullExtractor 2");
