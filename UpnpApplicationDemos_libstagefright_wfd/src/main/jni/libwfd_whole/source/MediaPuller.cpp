...
void MediaPuller::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatStart:
        {
            ALOGE("MediaPuller::onMessageReceived kWhatStart");
            status_t err;
            if (mIsAudio) {
        ...
        case kWhatStop:
        {
            ALOGE("MediaPuller::onMessageReceived kWhatStop");
        ...
        case kWhatPull:
        {
            ALOGE("MediaPuller::onMessageReceived kWhatPull");
            int32_t generation;
            ...
            if (mPaused) {
                ...
                ALOGE("MediaPuller::onMessageReceived kWhatPull mPaused");
            ...
            if (err != OK) {
                ALOGE("MediaPuller::onMessageReceived kWhatPull err, err %d", err);
                ...
                ALOGE("MediaPuller::onMessageReceived kWhatPull OK 1");
                int64_t timeUs;
                ...
                ALOGE("MediaPuller::onMessageReceived kWhatPull OK 2");

                if (mIsAudio) {
                    mbuf->release();
                    mbuf = NULL;
                    ALOGE("MediaPuller::onMessageReceived kWhatPull OK 3 audio");
                } else {
                    // video encoder will release MediaBuffer when done
                    // with underlying data.
                    accessUnit->setMediaBufferBase(mbuf);
                    ALOGE("MediaPuller::onMessageReceived kWhatPull OK 3 video");
                }
                ...
                notify->post();
                ALOGE("MediaPuller::send data kWhatAccessUnit size %d", accessUnit->size());
        ...
        case kWhatPause:
        {
            ALOGE("MediaPuller::onMessageReceived kWhatPause");
            mPaused = true;
            break;
        }
        ...
        case kWhatResume:
        {
            ALOGE("MediaPuller::onMessageReceived kWhatResume");
            mPaused = false;
            break;
        }
